...
function /s StimulusList()
	return Core#ListPackageInstances("Acq","stimuli",except="ch*")
end
...
Function /S AssemblerList(chan)
	...
	goodFuncs = sortlist(goodFuncs)
	variable index = WhichListItem(currFunc,goodFuncs)
	if(index>=0)
		goodFuncs = ReplaceListItem("["+currFunc+"]",goodFuncs,index)
	endif
	return goodFuncs
End
